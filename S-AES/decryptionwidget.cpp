...
void DecryptWidget::decrypt()
{
    ...
    if (isBinary && cipherText.length() == 16) {
        QString decryptedBinaryText = m_aes.decrypt(cipherText, key);
        ...
        for (int i = 0; i < cipherText.length(); i+=2) {
            ...
            QString decryptedBinaryText = m_aes.decrypt(binaryChar, key);

            // 将解密后的二进制字符串转换为ASCII编码字符串输出
            QChar asciiCharleft(decryptedBinaryText.mid(0,8).toInt(nullptr, 2));
            decryptedText += asciiCharleft;
            QChar asciiCharright(decryptedBinaryText.mid(8,8).toInt(nullptr, 2));
